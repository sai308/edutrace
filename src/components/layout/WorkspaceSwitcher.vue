...
import {
    ChevronsUpDown, Plus, Database, Check,
    Trash2, HelpCircle, Pencil
...
const handleCreate = async (data: { name: string; icon: string; copySettings?: boolean }) => {
    try {
...
const getIcon = (name?: string) => {
    ...
    // @ts-ignore
    return LucideIcons[name] || Database
}
        ...
        <SidebarMenuItem>
            <DropdownMenu>
                ...
                <DropdownMenuContent class="w-(--radix-dropdown-menu-trigger-width) min-w-56 rounded-lg"
                    ...
                    <DropdownMenuLabel class="text-xs text-muted-foreground uppercase tracking-wider flex justify-between items-center">
                        {{ $t('workspace.title') }}
                        <button @click="goToGuide" class="hover:text-primary transition-colors">
                    ...
                    <DropdownMenuItem 
                        ...
                        :class="{'focus:bg-primary/10 bg-primary/10': ws.id === currentWorkspaceId}"
                    ...
                    >
                        ...
                        <div v-if="ws.id !== 'default'" class="absolute right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-background/80 backdrop-blur-sm rounded-md shadow-sm border p-0.5" 
                             @click.stop>
                            <button 
