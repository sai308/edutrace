...
import { databaseService, DB_VERSION, DEFAULT_DB_NAME, type IDBCustomSchema } from './DatabaseService';
import { local as storage } from './StorageService';
import type { Workspace, CreateWorkspaceOptions, WorkspaceExportData } from '../types/workspaces';
...
const MAINTENANCE_STORES = ['meets', 'groups', 'tasks', 'marks', 'members', 'finalAssessments', 'modules'] as const;
        ...
        for (const id of workspaceIds) {
            ...
            try {
                db = await openDB<IDBCustomSchema>(workspace.dbName, DB_VERSION, {
                    upgrade: (db, oldVer, newVer, trans) => databaseService['initSchema'](db, oldVer, newVer, trans as any)
                });
