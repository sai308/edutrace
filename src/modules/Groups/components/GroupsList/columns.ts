...
export function createColumns(
    ...
    return [
        ...
        },
        {
            ...
            header: ({ column }) =>
                h(DataTableColumnHeader, { column, title: t('groups.table.name') }),
        ...
        },
        {
            ...
            header: ({ column }) =>
                h(DataTableColumnHeader, { column, title: t('groups.table.course') }),
            cell: ({ row }) =>
                h('span', { class: 'text-muted-foreground' }, row.getValue('course') ?? '-'),
        },
        {
            accessorKey: 'meetId',
            meta: { label: t('groups.table.meetId') },
            header: ({ column }) =>
                h(DataTableColumnHeader, { column, title: t('groups.table.meetId') }),
        ...
        },
        {
            ...
            header: ({ column }) =>
                h(DataTableColumnHeader, { column, title: t('groups.table.members') }),
            cell: ({ row }) =>
                h('span', { class: 'text-muted-foreground' }, getMemberCount(row.original.name)),
        },
        {
            accessorKey: 'teacher',
            meta: { label: t('groups.table.teacher') },
            header: ({ column }) =>
                h(DataTableColumnHeader, { column, title: t('groups.table.teacher') }),
            cell: ({ row }) =>
                h('span', { class: 'text-muted-foreground' }, row.getValue('teacher') ?? '-'),
        },
        {
            id: 'completion',
            ...
            cell: ({ row }) => {
                ...
                return h(
                    'div',
                    { class: ['text-center', completionClass(v)] },
                    v ? `${Math.round(v)}%` : '-',
                )
            },
        },
        ...
        },
        {
            ...
            header: ({ column }) =>
                h(
                    ...
                    [
                        h(DataTableColumnHeader, { column, title: t('groups.table.mode') }),
                        h(Star, { class: 'w-3 h-3' }),
                    ],
                ),
            cell: ({ row }) =>
