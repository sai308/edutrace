...
export function createSummaryColumns(
    ...
    const renderGradeTooltipContent = (gradeDetails: any) => {
        ...
        if (
            gradeDetails.missingTest ||
            (gradeDetails.missingTasks && gradeDetails.missingTasks.length > 0)
        ) {
            if (nodes.length > 0) {
            ...
            nodes.push(
                h('div', { class: 'text-xs font-semibold text-rose-500 mb-1.5' }, missingLabel),
            )
    ...
    modules.forEach((mod) => {
        cols.push({
            ...
            cell: ({ row }) => {
                ...
                const cleanGrade =
                    typeof gradeStr === 'string' ? gradeStr.replace('~', '') : gradeStr
    ...
    cols.push({
        ...
        cell: ({ row }) => {
            ...
            const displayTotal =
                !isNaN(parsed) && cleanTotal !== '' ? Math.round(parsed).toString() : cleanTotal
    ...
    cols.push({
        ...
        header: ({ column }) =>
            h(DataTableColumnHeader, { column, title: t('summary.completion') }),
        cell: ({ row }) => {
            ...
            return h(TooltipProvider, {}, () =>
                h(Tooltip, { delayDuration: 200 }, () => [
                    h(TooltipTrigger, { asChild: true }, () =>
                        h(
                            'span',
                            { class: `${colorClass} font-medium text-xs ml-4 cursor-help` },
                            str,
                        ),
                    ),
    ...
    cols.push({
        ...
        header: ({ column }) =>
            h(DataTableColumnHeader, { column, title: t('summary.attendance') }),
        cell: ({ row }) => {
            ...
            return h(TooltipProvider, {}, () =>
                h(Tooltip, { delayDuration: 200 }, () => [
                    h(TooltipTrigger, { asChild: true }, () =>
                        h(
                            'span',
                            { class: `${colorClass} font-medium text-xs mx-4 cursor-help` },
                            str,
                        ),
                    ),
                    h(TooltipContent, { class: 'text-xs' }, () => details),
    ...
    cols.push({
        ...
        cell: ({ row }) => {
            ...
            const hasGrade =
                row.original.examGrade !== null &&
                row.original.examGrade !== undefined &&
                row.original.examGrade !== ''
            ...
            return h(TooltipProvider, {}, () =>
                h(Tooltip, { delayDuration: 200 }, () => [
                    h(TooltipTrigger, { asChild: true }, () =>
                        h('div', { class: 'cursor-help w-max' }, badgeNode),
                    ),
    ...
    cols.push({
        ...
        header: ({ column }) =>
            h(DataTableColumnHeader, { column, title: t('summary.creditGrade') }),
        cell: ({ row }) => {
